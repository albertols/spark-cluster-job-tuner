...
-- DESCRIPTION EXPORT: Max concurrent jobs per cluster in window (approximate by minute sampling)
minutes AS (
  SELECT
    cluster_name,
    GENERATE_TIMESTAMP_ARRAY(
      TIMESTAMP_TRUNC(MIN(app_start_ts), MINUTE),
      TIMESTAMP_TRUNC(MAX(app_end_ts), MINUTE),
      INTERVAL 1 MINUTE
    ) AS minute_bins
...
FROM active
GROUP BY cluster_name
ORDER BY max_concurrent_jobs DESC;