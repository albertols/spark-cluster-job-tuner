...
| `divergenceShowcase` (multi-date) | 6 (`mock-cluster-show-*`) | 14 paired + 1 NEW on current | b16 OOM in both dates for `_DT4_OOM_RECURRING.json`; b16 in REF only for `_RDM_BOOST_HOLDING.json` | none | End-to-end demo for the z-score-driven features: compounded b16 boost (ReBoost), b16 boost holding, and divergence-driven executor scale-up; `_CTRL_NEWCOMER.json` shows the NEW pill in the divergences table |
| `durationDrift` (multi-date) | 1 (`mock-cluster-drift`) | 1 cap-pinned recipe `_DRIFT_DEMO.json`, 10m→25m blended (ratio 2.5, ≥10 min lost → Severe) | none | none | The censoring trap: trend-driven executor scaling (`ExecutorTrendScaler`) where the z-score path is blind |
| `capacityPressure` (multi-date) | 1 (`mock-cluster-capacity`, single-tenant) | 1 cap-saturated hog `_CAPACITY_HOG.json`, 20m→50m (ratio 2.5, ~29 min lost → Severe) | none | none | CapacityGuard end-to-end: trend pass inflates the ceiling, the FINAL guard pass clamps it (`capacityStatus: "clamped"`, red heatmap cell) |
| `trendPriority` (multi-date) | 1 (`mock-cluster-priority`) | 3 cap-pinned recipes: Critical 10m→60m (~49 min lost), Moderate 4m→9m (~4.9 min lost), Noise 20s→2m (ratio 6 but ~1.6 min lost → Negligible) | none | none | v2 severity tiers + impact-ranked grants: Critical outranks Moderate; Negligible HOLDS even when pinned |
