 ...
 *
 * These models capture the temporal dimension that the one-off tuner lacks: metrics across dates, performance trend
 * classification, evolution decisions, and statistical analysis results.
 */
 ...
 *
 * view = "delta": Pearson on (current - reference) values for paired entries only. view = "current_snapshot": Pearson
 * on raw current values, includes new entries. cluster = Some(name) when computed within a single cluster's recipes.
 */
 ...
 *
 * view = "delta": z-score on (current - reference) deltas. view = "current_snapshot": z-score on raw current values;
 * new entries flagged isNewEntry. clusterScope = Some(name) when stats were computed inside that cluster only.
 */
