...
object MockScenarios {
  ...
  //      `mock-cluster-show-oom` has recipe `_DT4_OOM_RECURRING.json` that
  ...
  def divergenceShowcase(refDate: String, curDate: String, seed: Long = 1234L): MultiDateScenario = {
    ...
    val refClusters: Seq[MockCluster] = Seq(
      ...
      MockCluster(
        ...
        recipes = Seq(
          recipeMedium("_DT4_OOM_RECURRING.json"),
          recipeLight("_DT4_OOM_COMPANION.json")
        ),
        incarnations = Seq(
          MockIncarnation(
            spanStart = s1.plus(2, ChronoUnit.HOURS),
        ...
        oomEvents = Seq(
          MockOomEvent(
            ...
            recipe = "_DT4_OOM_RECURRING.json",
    ...
    val curClusters: Seq[MockCluster] = refClusters.map { c =>
      c.name match {
        case "mock-cluster-show-oom" =>
          ...
          c.copy(
            recipes = c.recipes.map { r =>
              if (r.name == "_DT4_OOM_RECURRING.json")
                r.copy(
