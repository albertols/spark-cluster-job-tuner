...
```
etl-m-dwh-d-orbes-bultos-px-update-20260410-0517  →  (empty recipe)
etl-m-dwh-d-orbes-bultos-px-update-20260409-0514  →  _ETL_m_DWH_D_ORBES_BULTOS_PX_UPDATE.json
...
```
etl-m-dt4-ods-f-gt-borealis-20260411-0438
  → strip suffix  → etl-m-dt4-ods-f-gt-borealis
  → replace -→_   → etl_m_dt4_ods_f_gt_borealis
  → uppercase     → ETL_M_DT4_ODS_F_GT_BOREALIS
  → matches       → _ETL_m_DT4_ODS_F_GT_BOREALIS.json
```
    ...
    "entries": [
      {
        ...
        "cluster_name": "cluster-wf-dpr-load-t-03-25-0325",
        "raw_recipe_filename": "",
...
| [`ExecutorTrendVitamin`](/src/main/scala/com/db/serna/orchestration/cluster_tuning/single/refinement/RefinementVitamins.scala) | **Derived** — paired (reference, current) `RecipeMetrics` from the AutoTuner (NOT a CSV) | `spark.dynamicAllocation.{min,initial,max}Executors` (or `spark.executor.instances` for manual) | Longitudinal duration trend on a cap-pressured recipe — see [Trend-driven executor scaling](#trend-driven-executor-scaling) | Active (AutoTuner only) |
| [`CapacityGuardVitamin`](/src/main/scala/com/db/serna/orchestration/cluster_tuning/single/refinement/RefinementVitamins.scala) | **Derived** — the cluster's per-node capacity read from `clusterConf` scaled-max fields (NOT a CSV) | clamps `spark.dynamicAllocation.{min,initial,max}Executors` (or `spark.executor.instances`); stamps `maxCoreUsagePct` / `maxMemoryUsagePct` / `capacityStatus` | Always — a recipe's executor count exceeds `--max-cluster-util-ratio` (default 0.90) of the per-node-packed capacity. Runs as the AutoTuner's **final** pass after trend + z-score, so it caps whatever they inflated. No boost lifecycle (a physical ceiling, not a cumulative boost). | Active (AutoTuner only) |
...
The per-recipe decision flow (v2, magnitude-aware): blend → `classifySeverity` → `admittedStepCap` → cap-pressure gate → factor → creep rules; then a cluster-wide `prioritize` pass over the UP grants.

- **Driver:** a blended duration ratio `0.7·p95Ratio + 0.3·avgRatio` (each `current/reference`, guarded to `1.0` when either side is non-positive), plus the same blend expressed as **absolute minutes lost** (the delta).
- **Severity tiers (`classifySeverity`):** every tier above Negligible requires **BOTH** the relative ratio and the absolute minutes lost — a huge ratio on a 20-second job stays Negligible, and a 3-minute drift on a 10-hour job stays below the Severe ratio bar:

| Tier | Ratio condition | Absolute condition | Per-run step cap |
|---|---|---|---|
| Negligible | `< 1 + deadbandUp` **or** | delta `< minDeltaMinutes` (default 3 min) | never scales |
| Moderate | `≥ 1 + deadbandUp` **and** | delta `≥ minDeltaMinutes` | `maxStep` |
| Severe | `≥ 2.0` **and** | delta `≥ 10 min` | `maxStep × 1.5` |
| Critical | `≥ 3.0` **and** | delta `≥ 30 min` | `maxStep × 2.0` |

- **Graduated evidence (`admittedStepCap`):** with `runs = min(refRuns, curRuns)` — `runs ≥ minRunsForConfidence` (default 5) admits any non-Negligible tier at its full cap; 2–4 runs admit only Severe+ with the cap demoted one tier (Critical→Severe's cap, Severe→Moderate's cap); a single run admits only Critical, capped at ×1.5. Once admitted, confidence is floored at 0.5.
- **Cap-pressure gate:** `max(p95RunMaxExecutors / currentMax, fraction_reaching_cap)`. An admitted scale-**UP** still only fires when this is ≥ `capTouchRatio` (default 0.5) — a slowdown on a job that *isn't* parallelism-bound (plenty of idle executors) **Holds** instead.
...
- **Proportional sizing:** `max` tracks the ratio with `gain`, clamped per run by the admitted step cap and to cluster capacity with `min ≤ max − 1`; UP never shrinks the ceiling below the current `max`. **Min/initial creep:** `min` rises at most **+1 per run**, and only when the tier is Severe+ **and** cap-pressure ≥ 0.8; `initial` follows by at most +1 within `[min, max]` — the always-on floor rises far more conservatively than the ceiling.
- **Conservative scale-DOWN:** when a job speeds up *and* has cap headroom *and* clears the `downConfidenceFloor` *and* the blended saving is at least `minDeltaMinutes` (the symmetric absolute floor — never downscale to save seconds), `max` shrinks by `downGain` but never below observed peak demand (`p95RunMaxExecutors` plus `downSafetyMargin`) or the floor of 2; `min` shrinks at most **−1 per run**, never below steady demand (`avgExecutorsPerJob` plus margin). Disable entirely with `--no-trend-downscale`.
...
- **Cluster-wide prioritization (`ExecutorTrendScaler.prioritize`):** UP grants compete for a per-run budget of `upPoolRatio` (default 1.0) × the cluster's schedulable cores. Grants are ranked by `impactMinutes = Δminutes × current runs` (desc) and each draws `(newMax − originalMax) × executorCores` from the pool; when the pool is exhausted a grant degrades to `originalMax + 1` (never starved to zero) with its `reason` annotated. Holds/Downs pass through untouched. Each prioritized grant carries its 1-based `priorityRank`.

The trend path runs **before** the z-score `ExecutorScaleVitamin` and stamps its own `appliedTrendScaleFactor`, distinct from `appliedExecutorScaleFactor`, so the two mechanisms compose without lifecycle cross-talk — the combined effect on a recipe is the product of the two fields. Like the z-score path it uses the same `New`/`ReBoost`/`Holding` lifecycle and depends on [`BoostMetadataCarrier.carryTrendMetadata`](/src/main/scala/com/db/serna/orchestration/cluster_tuning/auto/BoostMetadataCarrier.scala) to survive a from-scratch cluster re-plan. The decision's `severity` / `impactMinutes` / `priorityRank` flow into the `executor_trend` boost group as per-recipe `severity`, `impact_minutes` and (for prioritized UP grants) `priority_rank`.
...
```json
"clusterConf": {
  "cluster-wf-dpr-load-t-02-15-0215": {
    "num_workers": 10,
    "worker_machine_type": "n2d-highcpu-48",
    ...
    "boostedMemoryHeapJobCount": 1,
    "boostedMemoryHeapJobList": ["_ETL_m_DT4_ODS_F_PV_NEBULOSA.json"],
...
```json
"_ETL_m_DT4_ODS_F_PV_NEBULOSA.json": {
