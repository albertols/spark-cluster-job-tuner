...
object RecipeResolver {
  ...
  /**
   * Strip date+time suffix from a job_id. e.g. "etl-m-dt4-ods-f-gt-borealis-20260411-0438" →
   * "etl-m-dt4-ods-f-gt-borealis"
   */
   ...
   * "etl-m-dt4-ods-f-gt-borealis" → "ETL_M_DT4_ODS_F_GT_BOREALIS"
   */
  private[refinement] def normaliseJobPrefix(prefix: String): String =
    prefix.replace('-', '_').toUpperCase

  /**
   * Normalise a recipe filename for case-insensitive matching. e.g. "_ETL_m_DT4_ODS_F_GT_BOREALIS.json" →
   * "ETL_M_DT4_ODS_F_GT_BOREALIS"
   */
