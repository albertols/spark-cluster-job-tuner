...
/**
 * Pure data describing a synthetic cluster fixture. No I/O — these case classes feed `MockGen` (writers) and are
 * instantiated by `MockScenarios` (prebuilt fixtures). All names use the obviously-fake convention (`mock-cluster-001`,
...
/**
 * Worker-count step function for one cluster incarnation. `schedule` is a list of (offset-seconds-from-spanStart,
 * target-primary-workers) pairs. The generator emits one b21 RECOMMENDING row per schedule entry, with `current`
...
final case class MockAutoscalerProfile(
  ...
  require(
    initialPrimary >= minPrimary && initialPrimary <= maxPrimary,
  ...
  schedule.foreach { case (off, target) =>
    ...
    require(target >= minPrimary && target <= maxPrimary, s"schedule target $target outside [$minPrimary, $maxPrimary]")
  }
}
...
/**
 * One incarnation interval of a cluster. The generator emits one b20 row per incarnation; `(spanStart, spanEnd]` is the
 * billing window. `events` may be empty (b23 avg-fallback path applies to cost) or carry the autoscaler decisions seen
 * during this incarnation. Multiple incarnations of the same cluster within `MockScenario.window` model "cluster
 * recreated" patterns.
 */
...
final case class MockCluster(
  ...
  require(
    incarnations.nonEmpty,
    s"MockCluster $name has no incarnations (need at least one — even if all are excludeFromB20)"
  )
  oomEvents.foreach { o =>
    require(
      recipes.exists(_.name == o.recipe),
...
/**
 * One generation pass for a single date. `window` bounds every incarnation's span and every autoscaler event. `seed`
 * makes any RNG-driven jitter (timestamps, decision strings) reproducible.
 */
final case class MockScenario(
  ...
  clusters.foreach { c =>
    c.incarnations.foreach { inc =>
      require(
        !inc.spanStart.isBefore(window._1) && !inc.spanEnd.isAfter(window._2),
...
/**
 * A coherent set of single-date scenarios, one per YYYY_MM_DD. The AutoTuner compares two of these (reference +
 * current) so multi-date drift produces meaningful trends/correlations/divergences for the frontend dashboard.
 */
