...
-- DESCRIPTION EXPORT: P95 job duration per recipe per cluster
SELECT
  cluster_name,
  recipe_filename,
  APPROX_QUANTILES(app_duration_millis, 100)[OFFSET(95)] AS p95_job_duration_ms,
  AVG(app_duration_millis) AS avg_job_duration_ms,
  COUNT(*) AS runs
FROM application_runs
GROUP BY cluster_name, recipe_filename
ORDER BY p95_job_duration_ms DESC;