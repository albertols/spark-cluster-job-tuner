...
/**
 * Candidate machine types considered by the tuner when computing cost-effective cluster shapes. This catalog now
 * includes Dataproc-compatible general-purpose machine families (Dataproc):
 * [[https://docs.cloud.google.com/compute/docs/general-purpose-machines]] N1, N2, N2D, E2, C3, C4, N4, and N4D, with
 * standard, highmem, and highcpu variants.
 * [[https://docs.cloud.google.com/dataproc/docs/concepts/compute/supported-machine-types]]
 *
 * Notes:
 *   - Memory per vCPU for most families is an integer multiple (e.g., 4/8/2 GB). We round to the nearest whole GB to
 *     keep MachineType.memoryGb as Int. This keeps sizing logic simple and deterministic.
 *   - The lists of vCPU counts are commonly-available shapes for each family; adjust if your region supports additional
 *     sizes.
 */
...
/**
 * A compact representation used in _clusters-summary.csv/json to visualize overall cluster activity and cost. Includes
 * dagId resolved from _dag_cluster-relationship-map.csv; if missing, uses "UNKNOWN_DAG_ID".
 */
...
/**
 * One billed lifetime of a Dataproc cluster (Create -> Delete). Same `cluster` may appear multiple times within a date
 * window with different `incarnationIdx`; (cluster, spanStart) is unique. Sourced from b20_cluster_span_time.sql.
 */
...
/**
 * One autoscaler decision for a cluster. The cost integrator only consumes events where `targetPrimary` is defined
 * (RECOMMENDING + real recommendation, i.e. SCALE_UP / SCALE_DOWN / NO_SCALE — CANCEL / DO_NOT_CANCEL drop because
 * `target_primary_workers` is NULL there). Sourced from b21_cluster_autoscaler_values.sql.
 */
...
/**
 * A single piecewise-constant segment of a cluster lifespan: between two adjacent autoscaler events (or the span
 * boundaries), workers is fixed and cost accrues linearly. Produced by `buildSpanSegments`. Used both for cost
 ...
 *
 *   - avgWorkers: global time-weighted average — Σ(workers × seg_seconds) / Σ(seg_seconds) over all segments of all
 *     spans (treats every interval as one weighted dataset).
 *   - minWorkers: max-of-per-span-mins — for each lifespan compute min(workers); take the max across lifespans (per
 *     requirement).
 *   - maxWorkers: max-of-per-span-maxes — for each lifespan compute max(workers); take the max across lifespans.
 */
...
/**
 * Composite output of `computeClusterCost` so single tuner and AutoTuner share one entry point. `costTimelineJson` is
 * `None` when no b20 spans exist (signals "no autoscaling data" to the frontend).
 */
...
object Sizing {
  ...
  def executorsPerWorker(
      worker: MachineType,
      executorCores: Int,
      executorMemoryGb: Int,
      memOverheadRatio: Double,
      reserveGb: Int
  ): Int = {
...
object ClusterMachineAndRecipeTuner {
  ...
  private val DagClusterMapPath: File = new File(
    "src/main/resources/composer/dwh/config/_dag_cluster-relationship-map.csv"
  )

  // Path to DAG/Cluster creation time map (CSV with headers: DAG_NAME,CLUSTER_NAME,TIMER_NAME,TIME,RUN_SINGLE_RECIPE_NUMBER)
  private val DagClusterCreationTimePath: File = new File(
    "src/main/resources/composer/dwh/config/_dag_cluster_creation_time.csv"
  )
   ...
   *   - inputDir/outputDir: where CSVs are read and JSON/CSV outputs are written, determined dynamically using the
   *     provided `date`.
  ...
  object Config {
    ...
    /**
     * Programmatic override used by the dashboard's TunerService, which has already resolved `inputsPath`/`outputsPath`
     * from `config.json`.
     */
  ...
  /**
   * Loads DAG_ID per cluster_name mapping from CSV. Returns Map[cluster_name -> dag_id]. If file missing or malformed
   * rows, returns empty map.
   */
  private[cluster_tuning] def loadDagClusterRelationshipMap(): Map[String, String] = {
    if (!DagClusterMapPath.exists()) {
      logger.warn(
        s"DAG-to-cluster relationship CSV not found: ${DagClusterMapPath.getPath}. All clusters will use dag_id=UNKNOWN_DAG_ID."
      )
      Map.empty
    } else {
  ...
  /**
   * Loads TIMER_NAME and TIME per CLUSTER_NAME from CSV _dag_cluster_creation_time.csv. Returns Map[cluster_name ->
   * (timer_name, time)]. If file missing or malformed rows, returns empty map. If multiple rows exist for the same
   * cluster, the first encountered is used with a warning.
   */
  private[cluster_tuning] def loadDagClusterCreationTimeMap(): Map[String, (String, String)] = {
    if (!DagClusterCreationTimePath.exists()) {
      logger.warn(
        s"TIMER mapping CSV not found: ${DagClusterCreationTimePath.getPath}. Defaulting TIMER_NAME=ZERO_TIMER, TIMER_TIME=00:00."
      )
      Map.empty
    } else {
      ...
      val pairs = rows.flatMap { r =>
        (r.get("CLUSTER_NAME"), r.get("TIMER_NAME"), r.get("TIME")) match {
          case (Some(cluster), Some(timer), Some(time)) if cluster.nonEmpty && timer.nonEmpty && time.nonEmpty =>
  ...
  /**
   *   - Reads b13_recommendations_inputs_per_recipe_per_cluster.csv or
   *     b15_recommendation_inputs_per_recipe_per_cluster.csv.
  ...
  private def loadFromIndividualCSVs(cfg: Config): Map[(String, String), RecipeMetrics] = {
    ...
    val b12 = readCsv("b12_p95_max_executors_per_recipe_per_cluster.csv").map { r =>
      val k = (r.getOrElse("cluster_name", ""), r.getOrElse("recipe_filename", ""))
      k -> (
        r.get("p95_run_max_executors").flatMap(Csv.toDouble),
        r.get("avg_run_max_executors").flatMap(Csv.toDouble),
        r.get("runs").flatMap(Csv.toLong)
      )
    }.toMap
   ...
   * Returns None for empty / "null" / unparseable input so missing endpoints (e.g. `delete_event_ts` for still-alive
   * incarnations) are tolerated.
   */
  ...
  /**
   * Load b20_cluster_span_time.csv produced from b20_cluster_span_time.sql. Returns one entry per (cluster_name) ->
   * chronologically-ordered incarnations. Spans whose start or end timestamp can't be parsed are dropped with a warn
   * log. Missing file is tolerated (returns empty map).
   */
  private[cluster_tuning] def loadClusterSpans(cfg: Config): Map[String, Seq[ClusterSpan]] = {
    ...
    if (!f.exists()) {
      logger.warn(
        s"b20 CSV not found: ${f.getPath}. estimated_cost_eur will be 0 for every cluster — produce this CSV from b20_cluster_span_time.sql to enable cluster-wall-clock cost."
      )
      return Map.empty
    }
    val rows = Csv.parse(f)
    val spans: Vector[ClusterSpan] = rows.flatMap { r =>
      ...
      (start, end) match {
        ...
        case _ =>
          logger.warn(
            s"Skipping b20 row with missing/invalid timestamps: cluster=$cluster idx=$idx start=${r.getOrElse("span_start_ts", "")} end=${r.getOrElse("span_end_ts", "")}"
          )
          None
      }
    }
  ...
  /**
   * Load b21_cluster_autoscaler_values.csv produced from b21_cluster_autoscaler_values.sql. Returns one entry per
   * (cluster_name) -> events sorted by event_ts ASC. Filters to RECOMMENDING events that carry a
   * `target_primary_workers` value, i.e. SCALE_UP / SCALE_DOWN / NO_SCALE — CANCEL / DO_NOT_CANCEL / COOLDOWN / SCALING
   * / INITIALIZING / STOPPED rows drop here because they don't move the cost step function. Missing file is tolerated
   * (returns empty map).
   */
  private[cluster_tuning] def loadAutoscalerEvents(cfg: Config): Map[String, Seq[AutoscalerEvent]] = {
    ...
    if (!f.exists()) {
      logger.warn(
        s"b21 CSV not found: ${f.getPath}. Autoscaler step-function cost (b22) is unavailable; spans without events will use the avg-fallback path (b23) which prices `clusterPlan.workers` flat across the span."
      )
      return Map.empty
    }
    val rows = Csv.parse(f)
    ...
    logger.info(
      s"Loaded ${events.size} autoscaler events across ${grouped.size} distinct clusters from ${f.getName} (filtered to RECOMMENDING + target)."
    )
    grouped
  }

  /**
   * Synthesize a single ClusterSpan from autoscaler events (b21) when b20 has no row for this cluster but b21 has
   * events. Span boundaries = [min(eventTs), max(eventTs)]. Marked `synthetic = true` so consumers (frontend / summary)
   * can render a badge. Returns `None` when fewer than 2 events (not enough to bound a span).
   */
  ...
  private[cluster_tuning] def hourlyPrice(machine: MachineType): Double =
    PriceCatalog.pricePerHourEUR.getOrElse(machine.name, 0.0)
   ...
   *
   * If `events` is empty (no autoscaler signal in this span) we follow the b23 "average fallback" path: a single
   * segment over the full span at `fallbackWorkers`.
   *
   * `events` MUST already be filtered to those falling within the span and sorted by eventTs ASC (loadAutoscalerEvents
   * enforces sort; caller filters by span via ClusterSpan.contains).
   */
  ...
  private[cluster_tuning] def clusterAutoscaleCostEur(
    ...
    buildSpanSegments(span, events, hourlyPrice(worker), hourlyPrice(master), fallbackWorkers).iterator
      .map(_.totalCostEur)
      .sum
  }
   ...
   *
   * Within a lifespan: walk segments and aggregate Σ(workers × seg_seconds) and Σ(seg_seconds); per-span min and max
   * are simple over segment workers. Across lifespans: divide global Σ to get the global time-weighted average;
   * minWorkers = max-of-per-span-mins and maxWorkers = max-of-per-span-maxes (per requirement — "max among the min" /
   * "max among the max").
   *
   * Spans without events use `fallbackWorkers` for the entire span (mirrors the b23 fallback path in cost integration).
   *
  ...
  /**
   * Build the JSON string embedded as `cost_timeline` in per-cluster JSON outputs. Returns `None` when no spans —
   * frontend uses absence as the "no autoscaling data exported" signal and renders the empty-state notice.
   */
  private[cluster_tuning] def costTimelineJson(
    ...
    val grandTotal = spans.iterator.map { span =>
      val spanEvents = events.filter(e => span.contains(e.eventTs))
      buildSpanSegments(span, spanEvents, workerHourly, masterHourly, fallbackWorkers.toDouble).iterator
        .map(_.totalCostEur)
        .sum
    }.sum
  ...
  /**
   * Single entry point used by both single tuner and AutoTuner so both paths share identical wall-clock-minutes, cost
   * integration, worker stats, and cost_timeline JSON. Spans/events that are empty produce zeros and `None` for the
   * cost_timeline (signals "no autoscaling data" to the frontend).
   */
  ...
  private def penalizedHourlyClusterCost(
      worker: MachineType,
      workers: Int,
      master: MachineType,
      policy: TuningPolicy
    ...
    val penalty: Double =
      if (workers > policy.preferMaxWorkers)
  ...
  private def chooseMachines(
    ...
    val evaluated = candidates.map { w =>
      val epw: Int = Sizing.executorsPerWorker(
        w,
        corePref,
        policy.executorMemoryGb,
        policy.memoryOverheadRatio,
        policy.osAndDaemonsReserveGb
      )
    ...
    val scored = evaluated.map {
      case (w, m, workers, epw, cost, totalCores, capacitySlots, coreSuff, util, oversize, shortfall, quotaOk) =>
    ...
    val pool =
      if (admissible.nonEmpty) admissible
    ...
    logger.info(
      f"Cluster '$clusterName' selection: reqSlotsBuffered=$reqSlots%.2f => " +
  ...
  def planCluster(
    ...
    val (workerType, masterType, workers, epwPref) =
      chooseMachines(clusterName, requiredSlotsRaw, policy, quotaTracker, pref)
    quotaTracker.recordCluster(workerType, workers, pref)

    val maxExecSupported: Int = workers * epwPref
    logger.info(
      s"Cluster '$clusterName': execsPerWorker=$epwPref, maxConc=$maxConc, targetExecPerJob=$targetExecPerJob, workers=$workers, maxExecSupported=$maxExecSupported"
    )
    ClusterPlan(clusterName, masterType, workerType, workers, epwPref, maxExecSupported)
  }

  def planManualRecipes(
      cluster: ClusterPlan,
      recipes: Iterable[RecipeMetrics],
      policy: TuningPolicy
  ): Seq[RecipePlanManual] = {
    val epw8: Int = Sizing.executorsPerWorker(
    ...
    recipes.toSeq.sortBy(-_.p95RunMaxExecutors).map { m =>
      ...
      logger.info(
        s"Manual plan ${cluster.clusterName}/${m.recipe}: base=$base boosted=$boosted desired=$desired capped=$capped cores=$cores memGb=$memGb (max8core=$maxEightCoreExecs)"
      )
      RecipePlanManual(
        recipe = m.recipe,
  ...
  def planDARecipes(cluster: ClusterPlan, recipes: Iterable[RecipeMetrics], policy: TuningPolicy): Seq[RecipePlanDA] = {
    recipes.toSeq.map { m =>
      ...
      logger.info(
        s"DA plan ${cluster.clusterName}/${m.recipe}: min=$minE initial=$initialE max=$maxE cores=${coresPreferred} memGb=$memGb (p95=${m.p95RunMaxExecutors})"
      )
      RecipePlanDA(
        recipe = m.recipe,
        minExecutors = minE,
  ...
  /**
   * Emits <cluster>-manually-tuned.json. Optional driverOverride injects driver resource fields into clusterConf when
   * YARN driver eviction has been detected for this cluster.
   */
  ...
  private[cluster_tuning] def writeCsv(outDir: File, fileName: String, rows: Seq[ClusterSummary]): Unit = {
    ...
    try {
      bw.write(
        "cluster_name,dag_id,no_of_jobs,num_of_workers,worker_machine_type,master_machine_type,real_used_avg_num_of_workers,real_used_min_workers,real_used_max_workers,total_active_minutes,estimated_cost_eur,TIMER_NAME,TIMER_TIME\n"
      )
      rows.foreach { s =>
        bw.write(
  ...
  private[cluster_tuning] def writeGlobalCoresAndMachinesCsv(outDir: File, summaries: Seq[ClusterSummary]): Unit = {
    ...
    try {
      bw.write(
        "MACHINE_TYPE, ESTIMATED_MAX_NO_OF_WORKERS, ESTIMATED_MAX_NO_OF_CORES, REAL_MAX_NO_OF_WORKERS, REAL_MAX_NO_OF_CORES, CLUSTERS_LIST\n"
      )

      val grouped = summaries.groupBy(_.workerMachineType)

      grouped.toSeq
        .sortBy { case (mt, clusters) =>
        ...
        .foreach { case (machineTypeName, clusterRows) =>
          ...
          bw.write(
            s"$machineTypeName,$estimatedWorkersSum,$estimatedCoresSum,$realMaxWorkersSum,$realMaxCoresSum,$clustersList\n"
          )
        }
  ...
  def run(cfg: Config, strategy: TuningStrategy = DefaultTuningStrategy): Unit = {
    logger.info(
      s"Starting ClusterMachineAndRecipeTuner. inputDir=${cfg.inputDir.getPath} outputDir=${cfg.outputDir.getPath} flattened=${cfg.useFlattened} strategy=${strategy.name}"
    )

    val metricsByKey: Map[(String, String), RecipeMetrics] = loadMetrics(cfg)
    if (metricsByKey.isEmpty) {
      logger.warn(
        "No metrics loaded. No JSONs will be produced. Ensure CSVs exist in the input directory and the query was executed."
      )
      return
    }
    ...
    metricsByCluster.foreach { case (clusterName, recMetrics) =>
      ...
      val clusterPlan: ClusterPlan =
        planCluster(clusterName, recMetrics, policy, quotaTracker, strategy.machinePreference)
      ...
      val spans: Seq[ClusterSpan] = if (rawSpans.isEmpty && events.nonEmpty) {
        synthesizeSpanFromEvents(clusterName, events) match {
          case Some(s) =>
            logger.info(
              s"No b20 span for $clusterName but ${events.size} b21 events found — synthesized span [${s.spanStart}, ${s.spanEnd}]."
            )
            Seq(s)
          case None =>
            logger.warn(
              s"No b20 span for $clusterName and only ${events.size} b21 event(s) — cannot synthesize span; estimated_cost_eur=0.0."
            )
            Seq.empty
        }
      } else rawSpans
      if (spans.isEmpty) {
        logger.warn(
          s"No b20 cluster span for $clusterName; estimated_cost_eur=0.0 and total_active_minutes=0.0. Provide b20_cluster_span_time.csv covering this cluster's active window to compute cost."
        )
      }
