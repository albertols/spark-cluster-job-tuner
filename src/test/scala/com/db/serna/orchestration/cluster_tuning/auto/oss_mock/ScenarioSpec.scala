...
/**
 * Referential-integrity tests on the prebuilt scenarios. These run without touching disk — they walk the in-memory
 * `MockScenario` graphs.
 *
 * Failing one of these means an existing scenario is internally inconsistent (e.g. an OOM event referencing a recipe
 * the cluster doesn't have) and would confuse downstream consumers.
 */
class ScenarioSpec extends AnyFunSuite with Matchers {
  ...
  private val prio = MockScenarios.trendPriority(refDate, curDate)
  private val scenarios: Seq[(String, MockScenario)] = Seq(
    ...
    "trendPriority-reference" -> prio.perDate(refDate),
    "trendPriority-current" -> prio.perDate(curDate)
  )
  ...
  test("multiDateBaseline emits exactly two dates, one shared and one drifted cluster set") {
    ...
    val refAvg001 = multi.perDate(refDate).clusters.find(_.name == "mock-cluster-001").get.recipes.head.avgJobDurationMs
    val curAvg001 = multi.perDate(curDate).clusters.find(_.name == "mock-cluster-001").get.recipes.head.avgJobDurationMs
    curAvg001 should be > refAvg001

    // mock-cluster-003 should be improved (smaller avgJobDurationMs in current).
    val refAvg003 = multi.perDate(refDate).clusters.find(_.name == "mock-cluster-003").get.recipes.head.avgJobDurationMs
    val curAvg003 = multi.perDate(curDate).clusters.find(_.name == "mock-cluster-003").get.recipes.head.avgJobDurationMs
  ...
  test("mixedDropAndDegrade degrades must-boost durations from reference to current") {
    val multi = MockScenarios.mixedDropAndDegrade(refDate, curDate)
    val refMustBoost = multi
      .perDate(refDate)
      .clusters
      .head
      .recipes
      .find(_.name == "mock-recipe-must-boost.json")
      .get
    val curMustBoost = multi
      .perDate(curDate)
      .clusters
      .head
      .recipes
  ...
  test("divergenceShowcase: OOM-recurring recipe takes a b16 hit on BOTH dates") {
    ...
    refOom.map(_.recipe) should contain("_DT4_OOM_RECURRING.json")
    curOom.map(_.recipe) should contain("_DT4_OOM_RECURRING.json")
  }
  ...
  test("divergenceShowcase: needs-execs recipe explodes ~40x on duration in current") {
    val refRecipe = showcase
      .perDate(refDate)
      .clusters
      .find(_.name == "mock-cluster-show-needs-execs")
      .get
      .recipes
      .find(_.name == "_DWH_NEEDS_MORE_EXECUTORS.json")
      .get
    val curRecipe = showcase
      .perDate(curDate)
      .clusters
      .find(_.name == "mock-cluster-show-needs-execs")
      .get
