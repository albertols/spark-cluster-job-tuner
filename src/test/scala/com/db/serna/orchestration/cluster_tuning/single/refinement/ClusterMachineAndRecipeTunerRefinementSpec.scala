...
class ClusterMachineAndRecipeTunerRefinementSpec extends AnyFunSuite with Matchers {
  ...
  private val sampleAutoScaleJson: String =
    """{
      ...
      |    },
      |    "_ETL_m_DT4_ODS_F_AC_OPE_AURORA.json": {
      |      "parallelizationFactor": 5,
      |      "sparkOptsMap": {
      |        "spark.serializer": "org.apache.spark.serializer.KryoSerializer",
      |        "spark.closure.serializer": "org.apache.spark.serializer.KryoSerializer",
  ...
  test("refine: applies heap boost to OOM-affected recipe and counts correctly") {
    ...
    // Write b16 CSV with one OOM for _ETL_m_DT4_ODS_F_PV_NEBULOSA.json
    ...
    try {
      ...
      pw.println(
        "job-1,cluster-wf-dpr-load-t-02-15-0215,_ETL_m_DT4_ODS_F_PV_NEBULOSA.json,2026-04-12T13:03:51Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,Java heap space,driver"
      )
    } finally pw.close()
    ...
    // Only NEBULOSA should be boosted
    result.appliedBoosts should have size 1
    result.boostCounters("boostedMemoryHeapJobCount") shouldBe 1
    result.boostLists("boostedMemoryHeapJobList") shouldBe Seq("_ETL_m_DT4_ODS_F_PV_NEBULOSA.json")

    // Boosted recipe
    val boosted = result.refinedRecipes("_ETL_m_DT4_ODS_F_PV_NEBULOSA.json")
    ...
    // Unaffected recipe
    val untouched = result.refinedRecipes("_ETL_m_DT4_ODS_TKN05.json")
  ...
  test("refine: no boosts when no OOM signals for cluster") {
    ...
    try {
      ...
      pw.println(
        "job-1,other-cluster,_ETL_formula_X.json,2026-04-12T13:00:00Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
  ...
  test("refine: multiple OOM recipes get boosted, counter and list reflect count") {
    ...
    try {
      ...
      pw.println(
        "job-1,cluster-wf-dpr-load-t-02-15-0215,_ETL_m_DT4_ODS_F_PV_NEBULOSA.json,2026-04-12T13:00:00Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
      pw.println(
        "job-2,cluster-wf-dpr-load-t-02-15-0215,_ETL_m_DT4_ODS_F_AC_OPE_AURORA.json,2026-04-12T14:00:00Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
    ...
    result.refinedRecipes("_ETL_m_DT4_ODS_F_PV_NEBULOSA.json").sparkOptsMap("spark.executor.memory") shouldBe "16g"
    result.refinedRecipes("_ETL_m_DT4_ODS_F_AC_OPE_AURORA.json").sparkOptsMap("spark.executor.memory") shouldBe "16g"
    result.refinedRecipes("_ETL_m_DT4_ODS_TKN05.json").sparkOptsMap("spark.executor.memory") shouldBe "8g"
  }
  ...
  test("refine: resolves empty recipe_filename via job-id derivation and boosts") {
    ...
    try {
      ...
      pw.println(
        "etl-m-dt4-ods-f-pv-nebulosa-20260409-0356,cluster-wf-dpr-load-t-02-15-0215,,2026-04-09T03:56:00Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
    ...
    result.appliedBoosts should have size 1
    result.appliedBoosts.head.recipeFilename shouldBe "_ETL_m_DT4_ODS_F_PV_NEBULOSA.json"
    result.refinedRecipes("_ETL_m_DT4_ODS_F_PV_NEBULOSA.json").sparkOptsMap("spark.executor.memory") shouldBe "12g"
    result.unresolvedEntries shouldBe empty
  }
  ...
  test("refine: resolves empty recipe_filename via sibling lookup") {
    ...
    try {
      ...
      pw.println(
        "etl-m-dt4-ods-f-ac-ope-aurora-20260409-0015,cluster-wf-dpr-load-t-02-15-0215,_ETL_m_DT4_ODS_F_AC_OPE_AURORA.json,2026-04-09T00:15:00Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
      pw.println(
        "etl-m-dt4-ods-f-ac-ope-aurora-20260410-0015,cluster-wf-dpr-load-t-02-15-0215,,2026-04-10T00:15:00Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
    ...
    result.appliedBoosts should have size 1
    result.appliedBoosts.head.recipeFilename shouldBe "_ETL_m_DT4_ODS_F_AC_OPE_AURORA.json"
    result.unresolvedEntries shouldBe empty
  }
  ...
  test("refine: unresolvable signals produce unresolvedEntries") {
    ...
    try {
      ...
      pw.println(
        "totally-unknown-job-20260411-0438,cluster-wf-dpr-load-t-02-15-0215,,2026-04-11T02:59:10Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
  ...
  test("refine: reports resolved signals whose recipe is not in the config") {
    ...
    try {
      ...
      pw.println(
        "etl-kcop,cluster-wf-dpr-load-t-02-15-0215,EL_KCOP.json,2026-03-16T06:46:38Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
  ...
  test("toRefinedJson: no duplicate boost counters on re-run") {
    ...
    try {
      ...
      pw.println(
        "job-1,cluster-wf-dpr-load-t-02-15-0215,_ETL_m_DT4_ODS_F_PV_NEBULOSA.json,2026-04-12T13:03:51Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
  ...
  test("toRefinedJson: produces JSON with boost counters in clusterConf") {
    ...
    json should include("\"_ETL_m_DT4_ODS_F_PV_NEBULOSA.json\"")
