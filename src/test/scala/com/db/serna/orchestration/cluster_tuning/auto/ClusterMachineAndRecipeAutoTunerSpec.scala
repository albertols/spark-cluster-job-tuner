...
class ClusterMachineAndRecipeAutoTunerSpec extends AnyFunSuite with Matchers {
  ...
  test("writeAutoTunerSummaryReport creates report with correct sections") {
    ...
    val b16Boosts = Seq(
      (
        ...
        Seq(
          ...
          MemoryHeapBoost("_ETL_formula_B.json", "16g", "24g", 1.5)
        )
      )
    )
    ...
    content should include("ETL_formula_A")
    content should include("8g -> 12g")
    content should include("ETL_formula_B")
  ...
  test("b16 reboosting persists from reference_date when b16 CSV absent in current_date") {
    ...
    writeCsv(
      ...
      B16Header + "\n" +
        "etl-m-formula-a-20260411-0438,cluster-a,_ETL_formula_A.json,2026-04-11T04:38:00Z,ERROR,SomeClass,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,Java heap space,log1"
    )
    ...
    signals.head.asInstanceOf[MemoryHeapOomSignal].recipeFilename shouldBe "_ETL_formula_A.json"
