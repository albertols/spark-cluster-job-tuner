...
class RefinementVitaminsSpec extends AnyFunSuite with Matchers {
  ...
  test("loadSignals: parses b16 CSV and returns signals for matching cluster") {
    ...
    try {
      ...
      pw.println(
        "job-1,cluster-a,_ETL_formula_A.json,2026-04-12T13:00:00Z,ERROR,some.Class,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,Java heap space,driver"
      )
      pw.println(
        "job-2,cluster-b,_ETL_formula_B.json,2026-04-12T14:00:00Z,ERROR,some.Class,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,Java heap space,driver"
      )
      pw.println(
        "job-3,cluster-a,_ETL_formula_C.json,2026-04-12T15:00:00Z,ERROR,some.Class,java.lang.StackOverflowError,FALSE,TRUE,FALSE,Stack overflow,driver"
      )
    } finally pw.close()
    ...
    s.recipeFilename shouldBe "_ETL_formula_A.json"
  ...
  test("loadSignals: includes signals with empty recipe_filename") {
    ...
    try {
      ...
      pw.println(
        "etl-m-dt4-ods-f-gt-borealis-20260411-0438,cluster-a,,2026-04-11T02:59:10Z,ERROR,main,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,OOM,driver"
      )
    } finally pw.close()
    ...
    signals.head.jobId shouldBe "etl-m-dt4-ods-f-gt-borealis-20260411-0438"
  }
  ...
  test("loadSignals: returns empty when no matching cluster") {
    ...
    try {
      ...
      pw.println(
        "job-1,cluster-b,_ETL_formula_B.json,2026-04-12T14:00:00Z,ERROR,some.Class,java.lang.OutOfMemoryError,FALSE,FALSE,TRUE,Java heap space,driver"
      )
    } finally pw.close()
  ...
  private val sampleRecipes: Map[String, RecipeConfig] = Map(
    "_ETL_formula_A.json" -> RecipeConfig(
    ...
    "_ETL_formula_B.json" -> RecipeConfig(
      parallelizationFactor = 5,
      sparkOptsMap = Map(
        "spark.executor.memory" -> "8g",
        "spark.executor.cores" -> "8",
  ...
  test("computeBoosts: produces boost for matching recipe") {
    val signals = Seq(
      MemoryHeapOomSignal("cluster-a", "_ETL_formula_A.json", "job-1", "2026-04-12T13:00:00Z", "OOM")
    )
    ...
    b.recipeFilename shouldBe "_ETL_formula_A.json"
  ...
  test("computeBoosts: returns empty when signal recipe not in config") {
    val signals = Seq(
      MemoryHeapOomSignal("cluster-a", "_ETL_missing.json", "job-99", "2026-04-12T13:00:00Z", "OOM")
    )
  ...
  test("computeBoosts: deduplicates multiple signals for same recipe") {
    val signals = Seq(
      MemoryHeapOomSignal("cluster-a", "_ETL_formula_A.json", "job-1", "2026-04-12T13:00:00Z", "OOM"),
      MemoryHeapOomSignal("cluster-a", "_ETL_formula_A.json", "job-2", "2026-04-12T14:00:00Z", "OOM")
    )
    val boosts = vitamin.computeBoosts(signals, sampleRecipes)
    boosts should have size 1
  }
  ...
  test("applyBoosts: updates spark.executor.memory and adds boost field") {
    val boosts = Seq(
      MemoryHeapBoost("_ETL_formula_A.json", "8g", "12g", 1.5)
    )
    val result = vitamin.applyBoosts(boosts, sampleRecipes)

    val rc = result("_ETL_formula_A.json")
  ...
  test("applyBoosts: recomputes total memory fields for manual recipe") {
    val boosts = Seq(
      MemoryHeapBoost("_ETL_formula_B.json", "8g", "12g", 1.5)
    )
    val result = vitamin.applyBoosts(boosts, sampleRecipes)
    val rc = result("_ETL_formula_B.json")
  ...
  test("applyBoosts: skip boost for recipe not in config") {
    val boosts = Seq(
      MemoryHeapBoost("_ETL_missing.json", "8g", "12g", 1.5)
    )
    val result = vitamin.applyBoosts(boosts, sampleRecipes)
    result shouldBe sampleRecipes
  }
  ...
  test("MemoryHeapBoostVitamin with factor 2.0 boosts 8g to 16g") {
    ...
    val signals = Seq(
      MemoryHeapOomSignal("cluster-a", "_ETL_formula_A.json", "job-1", "", "")
    )
  ...
  private val recipeNames = Set(
    "_ETL_m_DT4_ODS_F_GT_BOREALIS.json",
    "_ETL_m_DWH_D_ORBES_BULTOS_PX_UPDATE.json",
    "_ETL_m_DT4_ODS_F_PV_NEBULOSA.json"
  )

  test("RecipeResolver.stripJobIdSuffix removes date-time suffix") {
    RecipeResolver.stripJobIdSuffix("etl-m-dt4-ods-f-gt-borealis-20260411-0438") shouldBe "etl-m-dt4-ods-f-gt-borealis"
  }
  ...
  test("RecipeResolver.normaliseJobPrefix converts dashes to underscores and uppercases") {
    RecipeResolver.normaliseJobPrefix("etl-m-dt4-ods-f-gt-borealis") shouldBe "ETL_M_DT4_ODS_F_GT_BOREALIS"
  }

  test("RecipeResolver.normaliseRecipeName strips prefix and suffix") {
    RecipeResolver.normaliseRecipeName("_ETL_m_DT4_ODS_F_GT_BOREALIS.json") shouldBe "ETL_M_DT4_ODS_F_GT_BOREALIS"
  }

  test("RecipeResolver: resolves empty recipe via job-id derivation") {
    val signals: Seq[VitaminSignal] = Seq(
      MemoryHeapOomSignal("cluster-a", "", "etl-m-dt4-ods-f-gt-borealis-20260411-0438", "2026-04-11T02:59:10Z", "OOM")
    )
    val (resolved, unresolved) = RecipeResolver.resolve(signals, recipeNames)
    resolved should have size 1
    resolved.head.recipeFilename shouldBe "_ETL_m_DT4_ODS_F_GT_BOREALIS.json"
  ...
  test("RecipeResolver: resolves empty recipe via sibling lookup") {
    val signals: Seq[VitaminSignal] = Seq(
      MemoryHeapOomSignal(
        "cluster-a",
        "",
        "etl-m-dwh-d-orbes-bultos-px-update-20260410-0517",
      ...
      MemoryHeapOomSignal(
        "cluster-a",
        "_ETL_m_DWH_D_ORBES_BULTOS_PX_UPDATE.json",
        "etl-m-dwh-d-orbes-bultos-px-update-20260409-0514",
    ...
    resolved.head.recipeFilename shouldBe "_ETL_m_DWH_D_ORBES_BULTOS_PX_UPDATE.json"
    resolved(1).recipeFilename shouldBe "_ETL_m_DWH_D_ORBES_BULTOS_PX_UPDATE.json"
    unresolved shouldBe empty
  }
  ...
  test("RecipeResolver: signals with recipe already set pass through unchanged") {
    val signals: Seq[VitaminSignal] = Seq(
      MemoryHeapOomSignal("cluster-a", "_ETL_m_DT4_ODS_F_PV_NEBULOSA.json", "job-1", "", "")
    )
    val (resolved, unresolved) = RecipeResolver.resolve(signals, recipeNames)
    resolved should have size 1
    resolved.head.recipeFilename shouldBe "_ETL_m_DT4_ODS_F_PV_NEBULOSA.json"
    unresolved shouldBe empty
  }
  ...
  private val resolverRecipes: Map[String, RecipeConfig] = Map(
    "_ETL_m_DT4_ODS_F_GT_BOREALIS.json" -> RecipeConfig(
      parallelizationFactor = 5,
      sparkOptsMap = Map(
        "spark.executor.memory" -> "8g",
        "spark.executor.cores" -> "8",
  ...
  test("computeBoosts: produces boost for pre-resolved signal") {
    ...
    boosts should have size 1
    boosts.head.recipeFilename shouldBe "_ETL_m_DT4_ODS_F_GT_BOREALIS.json"
  }
}
