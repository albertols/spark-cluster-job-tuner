...
- **Trend-driven executor scaling:** `ExecutorTrendVitamin` + pure `ExecutorTrendScaler.decide` (`single/refinement/ExecutorTrendScaler.scala`, `ExecutorTrendVitamin.scala`) close the *censoring trap* — a job pinned at `maxExecutors` can't signal it needs more. Reads the per-recipe reference→current duration trend (blended `0.7·p95 + 0.3·avg`) and, when cap-pressured (`max(p95RunMax/max, fraction_reaching_cap) ≥ capTouchRatio`), resizes `min`/`initial`/`max` proportionally (`minGain < gain`; manual scales `instances`). Conservative scale-DOWN with hysteresis when jobs speed up (never below peak/steady demand; `--no-trend-downscale` to disable). Runs **before** the z-score pass; stamps `appliedTrendScaleFactor` — **distinct** from `appliedExecutorScaleFactor` so the two compose (combined = product). Gains derive from bias preset, overridable via `--trend-*` CLI flags. `BoostMetadataCarrier.carryTrendMetadata` carries prior min/initial/max + factor across re-plans. `SimpleJsonParser` round-trips `appliedTrendScaleFactor` in `extraFields`.
- **AutoTuner generation summary's `boost_groups`:** structured array of `{code, title, kind, count, count_new, count_holding, cluster_count, source?, entries}` consumed by the frontend. Codes today: `b14` (driver eviction), `b16` (heap OOM), `executor_scale` (z-score derived; `source: "derived"`, title `Z-score Executor SCALE-UP`, badge rendered as `z-score`), `executor_trend` (trend derived; `source: "trend"`, title `Trend Executor Scaling`, adds `count_up`/`count_down`). Adding a new vitamin = add a new entry here AND a CSS color in `frontend/style.css` for `.cluster-card .cluster-boost-chip.<code>` / `.detail-boost-group.<code>` / `.boost-card.<code>`.
